...
class DataSourceManager:
    ...
    async def _get_cache(self, key: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            # CacheClient.get 返回字符串或 None，需要 json.loads 反序列化
    ...
    async def _set_cache(self, key: str, data: Any, expire: int) -> None:
        ...
        """
        try:
            # CacheClient.set 使用 expire 关键字参数指定过期时间
            await self._cache.set(key, json.dumps(data, default=self._json_serializer), expire=expire)
    ...
    async def invalidate_cache(self, fund_code: str, cache_type: str = "all") -> None:
        ...
        if cache_type in ("nav", "all"):
            ...
            try:
                # 使用 CacheClient.keys 按模式匹配获取键列表
        ...
        if keys_to_delete:
            ...
            try:
                ...
                logger.info(f"已清除 {len(keys_to_delete)} 个缓存键")
            except Exception as e:

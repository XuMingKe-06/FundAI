...
class TushareAdapter(BaseDataSource):
    ...
    def __init__(self):
        """
        初始化 Tushare API 客户端（轻量化，不执行同步 API 调用）

        可用性检查已移至 check_availability() 异步方法，
        需要在初始化后显式调用以验证 API 权限。
        """
        super().__init__(name="tushare")

        # 优先从 config.json 读取（前端可编辑），回退到 .env 配置
        sm = get_settings_manager()
        self._token = sm.get("datasource.tushare_token") or settings.TUSHARE_TOKEN
        self._pro = None
        ...
        if self._token:
            try:
                ...
                # 仅标记客户端已创建，实际可用性由 check_availability() 验证
                self.is_available = True
                logger.info("Tushare API 客户端已创建，需调用 check_availability() 验证权限")
    ...
    async def check_availability(self) -> bool:
        """
        异步验证 Tushare API 是否可用（检查 fund_basic 接口权限）

        应在 __init__ 之后调用此方法，以确认 API 权限正常。
        此方法替代了原 __init__ 中的同步阻塞测试调用。
        ...
        try:
            test_df = await self._call_pro(
                self._pro.fund_basic, market='O', limit=1
            )
            if test_df is not None and not test_df.empty:
                self.is_available = True
                logger.info("Tushare API 可用性验证通过，fund_basic 接口可用")
            else:
                self.is_available = False
    ...
    async def _call_pro(self, func, **kwargs):
        """
        将同步 Tushare API 调用包装为异步执行

        Tushare 的 pro_api 所有方法都是同步阻塞调用，
        通过 asyncio.to_thread() 将其放入线程池执行，
        避免阻塞事件循环。

        Args:
            func: Tushare pro_api 的方法引用（如 self._pro.fund_basic）
            **kwargs: 传递给该方法的参数

        Returns:
            Tushare API 返回的 DataFrame
        """
    ...
    async def get_fund_info(self, fund_code: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            # 获取基金基本信息
            df = await self._call_pro(
                self._pro.fund_basic, ts_code=fund_code, fields=[
            ...
            scale = None
            try:
                scale_df = await self._call_pro(
                    self._pro.fund_share, ts_code=fund_code
                )
    ...
    async def search_funds(self, keyword: str, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            # 获取所有基金基本信息
            df = await self._call_pro(
                self._pro.fund_basic, market='E', fields=[
    ...
    async def get_holdings(self, fund_code: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            stock_holdings = []
            try:
                stock_df = await self._call_pro(
                    self._pro.fund_portfolio, ts_code=fund_code, fields=[
            ...
            bond_holdings = []
            try:
                bond_df = await self._call_pro(
                    self._pro.fund_portfolio, ts_code=fund_code, asset_type='b', fields=[
                        'ts_code', 'ann_date', 'end_date', 'symbol', 'name',
                        'amount', 'mkv', 'proportion'
                    ]
                )
    ...
    async def get_fund_manager(self, fund_code: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                mgr_df = await self._call_pro(
                    self._pro.fund_manager, ts_code=fund_code, fields=[
